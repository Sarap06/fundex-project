...
export default function InvestorDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // Check if investor has completed the survey
      ...
      if (!survey) {
        // Redirect to survey form if not completed
        router.push('/investor/survey');
        return;
      }
    ...
    <>
      <header className="bg-primary sticky top-0 z-30 border-b border-primary/80">
        <div className="px-8 py-5 flex justify-between items-center">
          <div>
            <h1 className="text-xl font-display font-bold text-white">Investor Dashboard</h1>
          ...
          <div className="bg-background rounded-lg shadow-lg p-8 border-t-4 border-primary">
            ...
            <div className="mt-8 p-4 bg-fundex-cream/30 border border-primary/20 rounded-lg">
              <p className="text-sm text-muted-foreground">
                Welcome to Fundex! You have been approved as an <strong>Investor</strong>. 
                You can now view investment opportunities and manage your portfolio.
              </p>
            </div>
          </div>
        )}
