...
export default function InvestorsPage() {
  ...
  if (loading) {
    return (
      <div className={`min-h-screen bg-neutral-50 transition-all duration-300 ${
        ...
        <header className={`bg-green-600 fixed top-0 right-0 z-30 transition-all duration-300 border-b border-green-700 ${
          ...
          <div className="px-8 py-5 flex justify-between items-center">
            <h1 className="text-xl font-bold text-white">Investors</h1>
          </div>
        </header>
        <main className="pt-20 px-8 py-8 flex items-center justify-center">
    ...
    <div className={`min-h-screen bg-neutral-50 transition-all duration-300 ${
      ...
      <header className={`bg-green-600 fixed top-0 right-0 z-30 transition-all duration-300 border-b border-green-700 ${
        ...
        <div className="px-8 py-5 flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold text-white">Investors</h1>
            <p className="text-xs text-green-100">Manage your investor network</p>
          </div>
          <button
      ...
      <main className="pt-20 px-8 py-8">
        <DashboardNav activeTab="investors" onTabChange={handleTabChange} isExpanded={isExpanded} onExpandChange={setIsExpanded} />
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <Card className="border-neutral-200"><CardContent className="p-5"><p className="text-sm text-neutral-600 mb-1">Total Investors</p><p className="text-3xl font-bold text-neutral-900">{stats.total}</p></CardContent></Card>
            <Card className="border-emerald-200 bg-gradient-to-br from-emerald-50 to-white"><CardContent className="p-5"><p className="text-sm text-emerald-700 mb-1">Active</p><p className="text-3xl font-bold text-emerald-600">{stats.active}</p></CardContent></Card>
            <Card className="border-neutral-200"><CardContent className="p-5"><p className="text-sm text-neutral-600 mb-1">Onboarding</p><p className="text-3xl font-bold text-blue-600">{stats.onboarding}</p></CardContent></Card>
            <Card className="border-neutral-200"><CardContent className="p-5"><p className="text-sm text-neutral-600 mb-1">Pending</p><p className="text-3xl font-bold text-amber-600">{stats.pending}</p></CardContent></Card>
          </div>
          ...
          <div className="mb-6">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search investors by name or email..." className="pl-10 h-11 bg-white border-neutral-300" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="flex gap-2">
                <Button variant="outline" className="gap-2"><Filter className="size-4" />Filter</Button>
                <Button className="gap-2 bg-emerald-600 hover:bg-emerald-700" onClick={() => setIsAddInvestorDrawerOpen(true)}><Plus className="size-4" />Add Investor</Button>
              </div>
            </div>

            <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
              {['all', 'active', 'onboarding', 'pending'].map((status) => (
                <button key={status} onClick={() => setStatusFilter(status)} className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === status ? 'bg-emerald-600 text-white' : 'bg-white text-neutral-700 border border-neutral-300 hover:bg-neutral-50'}`}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-neutral-50 border-b border-neutral-200">
                    <tr><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Investor</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Contact</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Sponsor</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Status</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Investments</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Total Invested</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Avg. Return</th><th className="text-left py-4 px-6 text-sm font-semibold text-neutral-900">Onboarded</th><th className="text-right py-4 px-6 text-sm font-semibold text-neutral-900">Actions</th></tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-100">
                    {filteredInvestors.length > 0 ? filteredInvestors.map((investor) => (
                      <tr key={investor.id} className="hover:bg-neutral-50 transition-colors">
                        <td className="py-4 px-6"><div className="flex items-center gap-3"><Avatar className="size-10"><AvatarFallback className="bg-gradient-to-br from-emerald-400 to-emerald-600 text-white font-semibold">{getInitials(investor.full_name)}</AvatarFallback></Avatar><div><p className="font-semibold text-neutral-900">{investor.full_name}</p><p className="text-sm text-neutral-500">{investor.investor_id}</p></div></div></td>
                        ...
                        <td className="py-4 px-6 text-neutral-900 font-medium">{investor.sponsor || 'Internal'}</td>
                        <td className="py-4 px-6"><Badge className={`${investor.status === 'Active' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' : investor.status === 'Onboarding' ? 'bg-blue-100 text-blue-700 hover:bg-blue-100' : 'bg-amber-100 text-amber-700 hover:bg-amber-100'}`}>{investor.status}</Badge></td>
                        <td className="py-4 px-6 text-neutral-900 font-medium">{investor.number_of_investments}</td>
                        <td className="py-4 px-6 text-neutral-900 font-semibold">${(investor.total_invested / 1000000).toFixed(1)}M</td>
                        <td className="py-4 px-6"><span className={`font-semibold ${investor.average_return ? 'text-emerald-600' : 'text-neutral-400'}`}>{investor.average_return ? `${investor.average_return.toFixed(1)}%` : '-'}</span></td>
                        <td className="py-4 px-6 text-sm text-neutral-600">{investor.onboarded_date}</td>
                        <td className="py-4 px-6"><div className="flex items-center justify-end gap-2"><Button variant="ghost" size="sm" className="text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50">View</Button><Button variant="ghost" size="icon"><MoreVertical className="size-4" /></Button></div></td>
                      </tr>
      ...
      {isAddInvestorDrawerOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full max-w-[480px] bg-white shadow-2xl overflow-y-auto">
            <div className="sticky top-0 bg-emerald-600 p-6 z-10"><div className="flex items-center justify-between"><h2 className="text-xl font-bold text-white">Add Investor</h2><button onClick={() => setIsAddInvestorDrawerOpen(false)} className="p-2 hover:bg-emerald-700 rounded-lg transition-colors"><X className="size-5 text-white" /></button></div></div>

            <form onSubmit={handleAddInvestor} className="p-6 space-y-6">
              <div className="space-y-4"><h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">Basic Information</h3><div><Label htmlFor="fullName">Full Name *</Label><Input id="fullName" placeholder="John Smith" required value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})} className="mt-1.5" /></div><div><Label htmlFor="email">Email Address *</Label><Input id="email" type="email" placeholder="john.smith@example.com" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="mt-1.5" /></div><div><Label htmlFor="phone">Phone Number</Label><Input id="phone" type="tel" placeholder="(555) 123-4567" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="mt-1.5" /></div></div>

              <div className="space-y-4"><h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">Investor Status</h3><div><Label htmlFor="status">Status *</Label><Select value={selectedStatus} onValueChange={(value) => setSelectedStatus(value)}><SelectTrigger className="mt-1.5" id="status"><SelectValue placeholder="Select status" /></SelectTrigger><SelectContent><SelectItem value="Active">Active</SelectItem><SelectItem value="Onboarding">Onboarding</SelectItem><SelectItem value="Pending">Pending</SelectItem></SelectContent></Select><p className="text-xs text-neutral-500 mt-1.5">Default: Onboarding</p></div></div>

              <div className="space-y-4"><h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">Sponsor</h3><div><Label htmlFor="sponsor">Sponsor</Label><Select value={selectedSponsor} onValueChange={(value) => {setSelectedSponsor(value); setShowAddSponsorInput(value === 'add-new');}}><SelectTrigger className="mt-1.5" id="sponsor"><SelectValue placeholder="Select sponsor" /></SelectTrigger><SelectContent>{sponsors.map((sponsor) => (<SelectItem key={sponsor.id} value={sponsor.company ? `${sponsor.name} – ${sponsor.company}` : sponsor.name}>{sponsor.company ? `${sponsor.name} – ${sponsor.company}` : sponsor.name}</SelectItem>))}<SelectItem value="add-new">+ Add New Sponsor</SelectItem></SelectContent></Select><p className="text-xs text-neutral-500 mt-1.5">Defaults to "Internal" if not selected</p></div>{showAddSponsorInput && <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg space-y-3"><div><Label htmlFor="newSponsorName">Sponsor Name *</Label><Input id="newSponsorName" placeholder="e.g., Derek" value={formData.newSponsorName} onChange={(e) => setFormData({...formData, newSponsorName: e.target.value})} className="mt-1.5" required={showAddSponsorInput} /></div><div><Label htmlFor="newSponsorCompany">Company (optional)</Label><Input id="newSponsorCompany" placeholder="e.g., 818 Consulting" value={formData.newSponsorCompany} onChange={(e) => setFormData({...formData, newSponsorCompany: e.target.value})} className="mt-1.5" /></div></div>}</div>

              <div className="space-y-4"><h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">Investment Details (Optional)</h3><div><Label htmlFor="initialInvestment">Initial Investment Amount ($)</Label><Input id="initialInvestment" type="number" placeholder="0" value={formData.initialInvestment} onChange={(e) => setFormData({...formData, initialInvestment: e.target.value})} className="mt-1.5" /></div><div><Label htmlFor="numInvestments">Number of Investments</Label><Input id="numInvestments" type="number" placeholder="0" value={formData.numberOfInvestments} onChange={(e) => setFormData({...formData, numberOfInvestments: e.target.value})} className="mt-1.5" /></div><div><Label htmlFor="notes">Notes</Label><Textarea id="notes" placeholder="Add any relevant notes about the investor..." rows={4} value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} className="mt-1.5" /></div></div>

              <div className="space-y-4"><h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">Tags (Optional)</h3><div className="flex flex-wrap gap-2">{['VIP', 'High Net Worth', 'New Investor', 'Accredited', 'Institutional'].map((tag) => (<button key={tag} type="button" onClick={() => {if (selectedTags.includes(tag)) {setSelectedTags(selectedTags.filter(t => t !== tag));} else {setSelectedTags([...selectedTags, tag]);}}} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${selectedTags.includes(tag) ? 'bg-emerald-100 text-emerald-700 border-emerald-300' : 'bg-white text-neutral-700 border-neutral-300 hover:bg-neutral-50'}`}><Tag className="size-3 inline mr-1" />{tag}</button>))}</div></div>
            </form>

            <div className="sticky bottom-0 bg-white border-t border-neutral-200 p-6"><div className="flex gap-3"><Button variant="outline" className="flex-1" onClick={() => setIsAddInvestorDrawerOpen(false)}>Cancel</Button><Button className="flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={handleAddInvestor} disabled={isSubmitting}>{isSubmitting ? <><Loader className="size-4 mr-2 animate-spin" />Creating...</> : <><Plus className="size-4 mr-2" />Create Investor</>}</Button></div></div>
          </div>
        </div>
      )}
    </div>
