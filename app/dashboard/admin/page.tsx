      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <p className="text-lg text-gray-900">Loading dashboard...</p>
          </div>
        </div>
      </div>
    );
      ...
      <main className="pt-20 px-8 py-8">
          ...
          <div className="space-y-6 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.values(stats).map((stat) => (
                <div key={stat.label} className="bg-white rounded-lg p-6 shadow-sm border border-gray-200 hover:shadow-md transition">
                  ...
                  <h3 className="text-xs font-medium text-gray-600 uppercase tracking-wide mb-1">{stat.label}</h3>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 text-sm">Add Investor</h3>
                  <p className="text-xs text-gray-600 mt-1">Onboard new investor</p>
                </button>
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 text-sm">Create Deal</h3>
                  <p className="text-xs text-gray-600 mt-1">Launch new offering</p>
                </button>
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 text-sm">Manage Allocations</h3>
                  <p className="text-xs text-gray-600 mt-1">Review capital deployment</p>
                </button>
                ...
                >
                  ...
                  <p className="text-xs text-gray-600 mt-1">Upload or review files</p>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Company Code Card */}
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <h2 className="text-lg font-bold text-gray-900 mb-4">Company Code</h2>
                <div className="bg-gray-50 rounded-lg p-4 flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 mb-1">Share this code with team members</p>
                    <p className="text-lg font-mono font-bold text-gray-900">{company?.company_code || 'N/A'}</p>
                  </div>
                  <button
                ...
                {codeMessage && (
                  <p className="text-xs text-green-600 mt-3">{codeMessage}</p>
                )}
              </div>

              {/* Invite Members Card */}
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <h2 className="text-lg font-bold text-gray-900 mb-4">Invite Team Members</h2>
                <div className="space-y-3">
                  ...
                  {inviteMessage && (
                    <p className={`text-xs ${inviteMessage.includes('sent') || inviteMessage.includes('copied') ? 'text-green-600' : 'text-red-600'}`}>
                      {inviteMessage}
                    </p>
                  )}
                </div>
                  ...
                  <tbody>
                    {members.map((member) => (
                      <tr key={member.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                        <td className="py-3 px-4 text-sm text-gray-900 font-medium">{member.full_name}</td>
            ...
            {pendingRequests.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <h2 className="text-lg font-bold text-gray-900 mb-6">Pending Join Requests</h2>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                    ...
                    <tbody>
                      {pendingRequests.map((request) => (
                        <tr key={request.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                          <td className="py-3 px-4 text-sm text-gray-900 font-medium">{request.full_name}</td>
                          ...
                          <td className="py-3 px-4 text-sm">
                            <select
                              ...
                              className="px-3 py-1 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500"
                            >
              ...
              <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-6 border-b border-gray-100">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => router.push('/dashboard/admin/deals')} className="text-teal-600 hover:text-teal-700 text-sm font-semibold">
                      View All
                    </button>
                  </div>
                </div>
                    ...
                    <div key={deal.id} className="p-6 hover:bg-gray-50 transition">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">{deal.id}</p>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          deal.status === 'Funding' 
                            ? 'bg-blue-100 text-blue-700'
                      ...
                      <div className="mb-3">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-xs font-semibold text-gray-900">{deal.progress}%</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                          <div 
                            className={`h-2.5 rounded-full ${
                              deal.status === 'Active' ? 'bg-teal-500' : 'bg-blue-500'
                            }`}
                      ...
                      <div className="flex justify-between pt-2 border-t border-gray-100">
                        <span className="text-xs text-gray-600">{formatCurrency(deal.raised)} of {formatCurrency(deal.target)}</span>
                        <span className="text-xs text-gray-600">{deal.investors} investors</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
                  {recentActivities.map((activity) => (
                    <div key={activity.id} className="p-4 hover:bg-gray-50 transition flex gap-3">
                      {activity.icon && activity.iconBgColorHex ? (
                        <ActivityIcon
                          icon={activity.icon}
                          bgColor={activity.iconBgColor || 'bg-gray-100'}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-teal-600 font-medium mt-0.5">{activity.dealName}</p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full mx-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-gray-900">Edit Member Role</h3>
                <button
                  onClick={() => {
                    setEditingMemberId(null);
                    setEditingRole('');
        ...
        {activeTab === 'investors' && (
          <div className="bg-white rounded-xl shadow p-12 text-center">
            ...
            <p className="text-gray-600">Investors management coming soon...</p>
          </div>
        )}
        ...
        {activeTab === 'allocations' && (
          <div className="bg-white rounded-xl shadow p-12 text-center">
            <DollarSign className="text-gray-400 mx-auto mb-4" size={64} />
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Allocations</h2>
            <p className="text-gray-600">Allocations management coming soon...</p>
          </div>
        )}
        ...
        {activeTab === 'documents' && (
          <div className="bg-white rounded-xl shadow p-12 text-center">
            <Plus className="text-gray-400 mx-auto mb-4" size={64} />
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Documents</h2>
            <p className="text-gray-600">Documents management coming soon...</p>
          </div>
        )}
      </main>
    </div>
