...
import { BroadcastAcknowledgmentStatus } from './BroadcastAcknowledgmentStatus';
      ...
      <div className="space-y-0">
        ...
        <div className="pb-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Deal Communications</h1>
              <p className="text-sm text-gray-600 mt-1">Official updates from your active deals.</p>
            </div>
        ...
        <div className="py-4 border-b border-gray-200">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:bg-white focus:border-green-500 focus:ring-1 focus:ring-green-500 text-sm"
            />
          </div>
        </div>
        ...
        {activeTab === 'deals' ? (
              ...
              <div className="space-y-2">
                {filteredDeals.map((deal) => (
                  <div
                    ...
                    className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg hover:shadow-md hover:border-green-300 cursor-pointer transition"
                  >
          ...
          <div className="space-y-3 pt-6">
            <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
              <AlertCircle className="mx-auto text-gray-400 mb-2" size={40} />
              <p className="text-gray-600">Investor inbox coming soon</p>
            </div>
          </div>
        )}
      </div>
  ...
  const renderDealDetailView = () => (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{selectedDeal?.name}</h1>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          ...
          className="px-4 py-2 bg-white border border-gray-300 text-gray-900 font-medium rounded-lg hover:bg-gray-50 transition flex items-center gap-2"
        >
          <FileText size={18} />
          Linked Docs
        </button>
        ...
        <button
          disabled
          className="px-4 py-2 bg-white border border-gray-300 text-gray-400 font-medium rounded-lg cursor-not-allowed opacity-50 flex items-center gap-2"
        >
          <AlertCircle size={18} />
          Pending Actions
        </button>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="p-6 space-y-4">
          ...
          <div className="border-b border-gray-200 pb-4 flex items-start justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-900">Latest Deal Update</h2>
              <p className="text-xs text-gray-600 mt-1">{selectedDeal?.name}</p>
            </div>
            <span
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-green-50 rounded-lg">
            <div>
              <p className="text-xs text-green-600 font-semibold uppercase mb-1">Target Amount</p>
              <p className="text-sm font-bold text-gray-900">
                ${parseFloat(selectedDeal?.target_amount || '0').toLocaleString('en-US', {
            ...
            <div>
              <p className="text-xs text-green-600 font-semibold uppercase mb-1">Raised Amount</p>
              <p className="text-sm font-bold text-green-600">
                ${parseFloat(selectedDeal?.raised_amount || '0').toLocaleString('en-US', {
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                })}
              </p>
            ...
            <div>
              <p className="text-xs text-green-600 font-semibold uppercase mb-1">Investors</p>
              <p className="text-sm font-bold text-blue-600">{selectedDeal?.investor_count || 0}</p>
            </div>
            <div>
              <p className="text-xs text-green-600 font-semibold uppercase mb-1">Progress</p>
              <p className="text-sm font-bold text-gray-900">{selectedDeal?.progress || 0}%</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div className="space-y-3">
              <div>
                ...
                <p className="font-medium text-gray-900">{selectedDeal?.type || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase mb-1">Close Date</p>
              ...
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase mb-1">Interest Rate</p>
                <p className="font-medium text-gray-900">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-medium text-gray-900">{selectedDeal?.location || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase mb-1">Term</p>
                <p className="font-medium text-gray-900">{selectedDeal?.term || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase mb-1">Borrower</p>
                <p className="font-medium text-gray-900">{selectedDeal?.borrower_name || 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="p-6">
          ...
          {timeline.length === 0 ? (
            ...
            <div className="space-y-3">
              {timeline.map((event) => (
                <div key={event.id} className="flex gap-4 pb-4 border-l-2 border-green-200 pl-4">
                  <div className="mt-2 w-3 h-3 bg-green-600 rounded-full -ml-6"></div>
                  <div className="flex-1">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-200 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Send Investor Update</h2>
        ...
        <form onSubmit={handleSendUpdate} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-900 mb-1.5">Update Title <span className="text-red-500">*</span></label>
            <input
              type="text"
              value={updateTitle}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-900 mb-1.5">Message Body <span className="text-red-500">*</span></label>
            <textarea
              value={updateMessage}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-900 mb-2">Document Attachments</label>
            <label className="flex flex-col items-center justify-center w-full px-4 py-5 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition">
              <div className="flex flex-col items-center justify-center">
                <Upload size={32} className="text-gray-400 mb-1" />
                <p className="text-xs font-medium text-gray-900">Click to upload documents</p>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-gray-700">Require investor acknowledgment</span>
          </label>
        </form>
        ...
        <div className="border-t border-gray-200 p-5 bg-gray-50 flex-shrink-0">
          <p className="text-xs text-gray-600 mb-3">
            <span className="font-semibold text-gray-900">{selectedDeal?.investor_count || 0} investors</span> will receive this update
          </p>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={handleGoBack}
              className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg text-gray-900 font-medium hover:bg-gray-100 transition"
            >
              Cancel
            </button>
            <button
              ...
              disabled={sending}
              className="flex-1 px-3 py-2 text-sm bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
  ...
  const renderScheduleUpdateView = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-200 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Schedule Future Update</h2>
            <p className="text-xs text-gray-600 mt-0.5">
        ...
        <form onSubmit={handleScheduleUpdate} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-1.5">Send Date <span className="text-red-500">*</span></label>
              <input
                type="date"
                value={scheduleDate}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-1.5">Send Time <span className="text-red-500">*</span></label>
              <input
                type="time"
                value={scheduleTime}
        ...
        <div className="border-t border-gray-200 p-5 bg-gray-50 flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={scheduling}
              className="flex-1 px-3 py-2 text-sm bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center justify-center gap-2"
            >
              <Calendar size={16} />
  ...
  const renderLinkedDocsView = () => (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Linked Documents</h1>
      </div>
      ...
      <div className="space-y-4">
        {documents.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <p className="text-gray-600">No documents linked to this deal</p>
          </div>
        ) : (
          <div className="space-y-2">
            {documents.map((doc) => (
              <a
                ...
                className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg hover:shadow-md hover:border-blue-300 transition"
              >
                <div className="flex items-center gap-3">
                ...
                <Download className="text-gray-400 hover:text-blue-600 transition" size={20} />
              </a>
            ))}
          </div>
        )}
  ...
  if (loading && currentView === 'channels') {
    return (
      <div className="text-center py-8">
        <Clock className="animate-spin mx-auto text-gray-600 mb-3" size={32} />
        <p className="text-gray-600">Loading broadcast channels...</p>
      </div>
    );
  }
