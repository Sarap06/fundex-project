...
export function SecurityControl() {
  const services = [
    { icon: <Shield className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Bank-Grade Security', description: 'SOC 2 Type II compliant infrastructure with end-to-end encryption for all loan data and investor information.' },
    { icon: <Lock className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Role-Based Access', description: 'Granular permissions for every team member. Control who sees what, from deal pipeline to investor details.' },
    { icon: <Eye className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Audit Trail', description: 'Complete visibility into every action taken on your platform. Full compliance-ready audit logging.' },
    { icon: <FileCheck className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Regulatory Compliance', description: 'Built-in compliance workflows for state and federal lending regulations. Stay ahead of regulatory changes.' },
    { icon: <Users className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Investor Verification', description: 'Automated accredited investor verification and KYC/AML checks integrated directly into onboarding.' },
    { icon: <BarChart3 className="w-5 h-5" style={{ color: 'var(--primary)' }} />, title: 'Real-Time Monitoring', description: 'Live dashboards for portfolio health, risk metrics, and performance analytics across all your deals.' },
  ];

  const stats = [
    { icon: <Award className="w-5 h-5" style={{ color: 'var(--primary)' }} />, value: 99, label: 'Uptime SLA', suffix: '.9%' },
    { icon: <Shield className="w-5 h-5" style={{ color: 'var(--primary)' }} />, value: 256, label: 'Bit Encryption', suffix: '' },
    { icon: <Users className="w-5 h-5" style={{ color: 'var(--primary)' }} />, value: 500, label: 'Active Firms', suffix: '+' },
    { icon: <CheckCircle className="w-5 h-5" style={{ color: 'var(--primary)' }} />, value: 100, label: 'Compliance Rate', suffix: '%' },
  ];
      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12">
        <div className="text-center mb-16">
          <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl font-black leading-tight text-foreground uppercase tracking-tight mb-4">
            Security & <span style={{ color: 'var(--primary)' }}>Control</span>
          </h2>
              ...
              <motion.div key={s.title} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <span className="inline-flex items-center gap-1 font-sans text-sm mt-2 opacity-0 group-hover:opacity-100 gentle-animation" style={{ color: 'var(--primary)' }}>Learn more <ArrowRight className="w-3 h-3" /></span>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <div className="lg:col-span-1 flex items-center justify-center">
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative rounded-2xl overflow-hidden w-full h-80 lg:h-full">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                ...
                <p className="font-sans text-white/80 text-sm">Bank-grade protection for your lending operations</p>
              </div>
            </motion.div>
          </div>
