      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12">
        <div className="text-center mb-16">
          <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl font-black leading-tight text-foreground uppercase tracking-tight mb-4">
            Who It's <span style={{ color: 'var(--primary)' }}>For</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto mb-20">
          {cases.map((c, i) => (
            ...
            >
              <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-5 text-white" style={{ backgroundColor: 'var(--primary)' }}>
                {c.icon}
              </div>
              <h3 className="font-display font-black text-xl text-foreground mb-3">{c.title}</h3>
              <p className="font-sans text-muted-foreground leading-relaxed mb-5">{c.description}</p>
              ...
              <span className="inline-flex items-center gap-2 font-sans text-sm font-semibold opacity-0 group-hover:opacity-100 gentle-animation cursor-pointer" style={{ color: 'var(--primary)' }}>
